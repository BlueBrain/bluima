 ...
 * methods to extract neuroanatomical connectivity statements from free text".
 * 
...
public class WhiteTextConnectionsCollectionReader extends
    ...
    @ConfigurationParameter(name = PARAM_ADD_NEG_PAIRS, defaultValue = "false", mandatory = false)
    ...
    private String addAnnotations(JCas jcas, List<Element> sentences) {
        ...
        for (Element sentence : sentences) {
            ...
            for (Element pair : sentence.getChildren("pair")) {
                ...
                if (addNegativePairs) {
                    cooc.setConfidence(hasInteraction ? 1.0f : 0.0f); // hack
                    cooc.addToIndexes();
