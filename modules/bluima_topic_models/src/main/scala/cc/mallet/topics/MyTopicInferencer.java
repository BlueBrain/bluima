...
public class MyTopicInferencer implements Serializable {
    ...
    public MyTopicInferencer (int[][] typeTopicCounts, int[] tokensPerTopic, Alphabet alphabet,
    ...
    public double[] getSampledDistribution(Instance instance, int numIterations,
                                           int thinning, int burnIn) throws IOException {

        TextFileWriter w = new TextFileWriter(ii+++".txt");
        ...
        for (int iteration = 1; iteration <= numIterations; iteration++) {
            ...
            w.addLine("iter "+iteration);
            //  Iterate over the positions (words) in the document                                                        
            for (int position = 0; position < docLength; position++) {
                type = tokens.getIndexAtPosition(position);
                
                Object objectAtPosition = tokens.getObjectAtPosition(position);
                ...
                String word = tokens.get(position).toString();
                if (iteration % 1000==1)
                w.addText(word +"["+newTopic+"] ");
