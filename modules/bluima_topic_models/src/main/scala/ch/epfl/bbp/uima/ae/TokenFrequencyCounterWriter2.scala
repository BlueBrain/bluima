...
/**
 * Counts the total number of occurrences of ShortestPaths.
 *
...
class TokenFrequencyCounterWriter2 extends JCasAnnotator_ImplBase {
  ...
  override def process(doc: JCas) {
    ...
    // /val casTokens = JCasUtil.select(doc, classOf[ShortestPath])
    val tokensStr = casTokens.map(t =>
      t.getCoveredText.toLowerCase.replaceAll("[\n\r\t]", ""))
