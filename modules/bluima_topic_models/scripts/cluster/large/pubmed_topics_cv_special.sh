...
i=0
logfile="$HOME/private/logs/pubmed_cv_fold$i_special.log"
rm $logfile
...
#	cp -r $working_dir "$corpus/dca$topics"
	...
	cd $estimator_path
	java -Xmx12000M  -Dblue_uima_home=$HOME/private/blue_uima_trunk -jar $HOME/private/software/sbt/sbt-launch-0.12.0.jar "run-main ch.epfl.bbp.uima.topicmodels.estimator.DCAModelEstimation -t $corpus_size -c $est_cycles $working_dir dca $corpus/test$i.txt" >> $logfile
	cd $working_dir
    done
done
