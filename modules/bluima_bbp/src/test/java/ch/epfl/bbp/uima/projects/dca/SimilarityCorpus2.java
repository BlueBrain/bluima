 ...
 * @author richarde
 */
public class SimilarityCorpus2 {

    public static void main(String[] args) throws Exception {

        // final int NR_TOPICS = 100;
        // final String ROOT =
        // "/Volumes/HDD2/ren_data/data_hdd/__mycorpora/1m_ns/";
        ...
        final String idsFile = ROOT + "1m_ns.pmids.txt";
        final int NR_TOPICS = 100;
        final String DOCTOPICS = ROOT + "trained__100/1m_ns.doctopics";
        ...
        // read topics
        Map<Integer, BasicVector> pmid2topicdist = newHashMap();
        ...
        for (int l = 0; l < doctopics.size(); l++) {
            ...
            pmid2topicdist.put(pmId, new BasicVector(topicdist));
        }
        ...
        // normalize
        Vector topicdistSum = new BasicVector(NR_TOPICS);
        for (Vector topicdist : pmid2topicdist.values()) {
            topicdistSum = topicdistSum.add(topicdist);
        }

        final Vector topicdistAvg = topicdistSum
                .divide(pmid2topicdist.size() + 0d);

        final BasicVector theTopicDist = new BasicVector(pmid2topicdist.get(
                THE_PMID).hadamardProduct(topicdistAvg));

        System.out.println("sum\t" + topicdistSum);
        System.out.println("avg\t" + topicdistAvg);
        System.out.println(THE_PMID + "\t" + pmid2topicdist.get(THE_PMID));
        System.out.println(theTopicDist);
        checkNotNull(theTopicDist);

        Map<Integer, Double> pmid_distances = newHashMap();
        int progress = 0;
        for (Entry<Integer, BasicVector> pmid_pz : pmid2topicdist.entrySet()) {

            final BasicVector pz_normalized = new BasicVector(pmid_pz
                    .getValue().hadamardProduct(topicdistAvg));
        ...
        for (Entry<Integer, Double> pmid_distance : sortByValue(pmid_distances)
            ...
            System.out.println(pmid + "\t" + pmid2topicdist.get(pmid));
