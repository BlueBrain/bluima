 ...
 * the original <strong>token</strong>.
 * 
 * @author renaud.richardet@epfl.ch
 */
@OperationalProperties(multipleDeploymentAllowed = false)
public class PreprocessingStatsAnnotator2 extends JCasAnnotator_ImplBase {

    // k: word, v: pair{ cnt, {list of words where word appear, max 20}};
    ...
    public void process(JCas jCas) throws AnalysisEngineProcessException {
        ...
        // String source = JCasUtil.selectSingle(jCas,
        // Header.class).getSource();
        ...
        for (Keep sp : select(jCas, Keep.class)) {
            ...
            if (pmIds.size() < 3) {
                String snippet = inspect(sp);
                pmIds.add(pmId + ": " + snippet);
            }
    ...
    public void collectionProcessComplete()
        ...
        try {
            TextFileWriter writer = new TextFileWriter(new File(
                    "target/preprocessing2_" + currentTimeMillis() + ".tsv"));
