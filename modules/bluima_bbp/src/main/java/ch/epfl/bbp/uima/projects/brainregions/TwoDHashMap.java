...
/**
 * A two-dimensional hashmap, is a HashMap that enables you to refer to values
 * via two keys rather than one. The underlying implementation is simply a
 * HashMap containing HashMap, each of which maps to values.
 * <P>
 * This is quite useful e.g. when wanting to cache values such as
 * "currency ration" based keys "day of year" and "year".
 * 
...
public class TwoDHashMap<K1, K2, V> {
     ...
     * @param secondKey
     *            second key
     * @return true when an element (or <tt>null</tt>) has been stored with the
     *         keys
     */
     ...
     *            second key
     * @param value
     *            the value to be inserted. <tt>null</tt> may be inserted as
     *            well.
     ...
     * 
     * @return Returns the number of key-value mappings in this map for the
     *         first key.
     */
    public int size() {
        return map.size();
    ...
    /** @return the key-value mappings for the second key */
    public Map<K2, V> getInnermap(final K1 firstKey) {
